include::locale/attributes.adoc[]
= Spesifikasjon for klassifikasjonsbeskrivelser (XKOS-AP-NO)
:doctype: book
:docinfo:
:icons: font
:toc: left
ifdef::backend-pdf[:toc: macro]
:toc-title: Innholdsfortegnelse
:toclevels: 4
:sectlinks:
:table-stripes: even
:title-logo-image: image:images/digitaliseringsdirektoratet.png[pdfwidth=30vw]

image::images/digitaliseringsdirektoratet.png[width=50%, pdfwidth=30vw]

include::shared/download.adoc[]

NOTE: *Innmelding av feil og mangler:* +
Dersom du finner feil eller mangler i dokumentet, ber vi om at dette meldes inn på https://github.com/Informasjonsforvaltning/xkos-ap-no/issues[Github Issues]. Dersom du ikke allerede har bruker på Github kan du opprette bruker gratis.

*Status*: [yellow-background]#til kommentering# +
*Versjon*: [yellow-background]#0.8# +
*Publisert*: 2021-11-18 +
*Oppdatert*: 2021-11-19 +
*Gjeldende versjon*: <ingen> +
*Forrige versjon*: <ingen> +
*Redaktørens utkast*: https://informasjonsforvaltning.github.io/xkos-ap-no/ +
*Veileder*: <ingen> +
*Valideringsverktøy*: https://data.norge.no/validator (ved å bruke https://raw.githubusercontent.com/Informasjonsforvaltning/xkos-ap-no/develop/shacl/xkos-ap-no-shacl-0x.ttl[denne shacl-filen] ([yellow-background]#p.t. under utvikling&testing#))

include::Om_denne_versjonen.adoc[]

toc::[]

include::Om_denne_spesifikasjonen.adoc[]

include::Innledning.adoc[]

include::Forenklet_modell.adoc[]

include::Klassifikasjon.adoc[]

include::Klassifikasjonsnivå.adoc[]

include::Kategori.adoc[]

include::Klassifikasjonssammenligning.adoc[]

include::Kategorisammenligning.adoc[]

include::Kontrollerte_vokabularer.adoc[]

include::Vedlegg.adoc[]



include::shared/stats.adoc[]
